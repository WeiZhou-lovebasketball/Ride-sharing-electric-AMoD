...
class Veh(object):
    ...
    def __init__(self, id: int, station: int, location: float, n_soc: int):
        ...
        # self.decharge = 10 / 1080  # 6 min for 1 kWh, 3.33 % 30 kWh
        # self.charge = 10 / 180    # fast charging 60 kW, 24 min for 80 % (24 kWh), 1 min 3.33 %
        # self.decharge = 2.5 / 360  # 6 min for 1 kWh, 2.5 % 40 kWh
        # self.charge = 2.5 / 60    # fast charging 60 kW, 32 min for 80 % (32 kWh), 1 min 2.5 %
        self.n_soc = n_soc
    ...
    def move(self):
        ...
        if self.status == VehicleStatus.CHARGING:
            ...
            # self.energy_charged += self.charge * 40 / 100
            if self.battery > 99.99:
                self.battery = 99.99
                return
