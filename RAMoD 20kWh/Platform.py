...
class Simulator(object):
    ...
    def __init__(self, fleet_size: int, num_stations: int, soc: int, horizon: int, request_path):
        ...
        self.vehs = []
        self.vehs_status = [[fleet_size - 20], [0], [0], [0], [20 * self.n_v], [0]]
    ...
    def get_result(self):
        ...
        df1.to_csv('result/request status 200 vehicle.csv', index=False)
        df2.to_csv('result/vehicle status 200 vehicle.csv', index=False)
        df3.to_csv('result/number of vehicle status 200 vehicle.csv', index=False)
        df4.to_csv('result/system energy 200 vehicle.csv', index=False)
        df5.to_csv('result/charged energy 200 vehicle.csv', index=False)