...
class Simulator(object):
    ...
    def update_vehs(self, last, now, period, flag):
        ...
        for veh in self.vehs:
            if veh.status == VehicleStatus.CHARGING and flag and last-period < veh.charging_start_t <= last:
            ...
            if veh.status == VehicleStatus.IDLE and last <= veh.arrival_time < now:
                if veh.battery < 0:
                    vaccant_vehs[veh.pos][0].append(veh)
                else:
                    vaccant_vehs[veh.pos][int(veh.battery * self.soc / 100)].append(veh)
                continue
    ...
    def read_vehs(self, interval):
        num_charging_vehs = np.zeros((self.n, self.soc))
        ...
        for veh in self.vehs:
            if veh.status == VehicleStatus.CHARGING:
                num_charging_vehs[veh.pos, int(veh.battery * self.soc / 100)] += 1
                continue
            if veh.status == VehicleStatus.IDLE:
                if veh.battery < 0:
                    num_vaccant_vehs[veh.pos, 0, 0] += 1
                else:
    ...
    def matching(self, popup_reqs: list[list[Req]], idle, r, c, x_zo, x_so, p_so, p_zo, new_vac_vehs: list[list[list[Veh]]], new_occ_vehs: list[list[list[Veh]]]):
        ...
        for i in range(self.n):
            ...
            for b in range(self.soc):
                ...
                # Secondly, match the vehicles which pick up only one customer to reqeusts
    ...
    def get_result(self):
        ...
        charged_energy = {'Time': T, 'charged energy in vehicles': self.vehs_status[5]}
        ...
        df1.to_csv('result/request status 200 vehicle copy.csv', index=False)
        df2.to_csv('result/vehicle status 200 vehicle copy.csv', index=False)
        df3.to_csv('result/number of vehicle status 200 vehicle copy.csv', index=False)
        df4.to_csv('result/system energy 200 vehicle copy.csv', index=False)
        df5.to_csv('result/charged energy 200 vehicle copy.csv', index=False)